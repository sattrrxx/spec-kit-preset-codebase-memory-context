...
- **Repository alignment inventory** (only when codebase context exists): Record the documented modules, package locations, persistence/data-access integration points, interface conventions, and validation commands relevant to the feature
...
- Check whether planned validation commands are supported by current build configuration, CI configuration, or repository documentation; do not execute the commands
- Treat an explicitly documented architecture change as intentional, and verify that tasks include the migration, compatibility, and validation work needed for it
- When a potential conflict matters to the report, corroborate it with current repository evidence using an available code graph or targeted repository/file inspection
- Record whether each finding is context-only or corroborated by current repository evidence
...
**Repository Alignment Issues:** (if any; identify each as context-only or repository-corroborated)
...
- Repository Alignment Issues Count
...
- Provide explicit command suggestions: e.g., "Run __SPECKIT_COMMAND_SPECIFY__ with refinement", "Run __SPECKIT_COMMAND_PLAN__ to adjust architecture or repository alignment", "Manually edit tasks.md to add coverage for 'performance-metrics'"
...
- **Skip repository alignment silently when context is absent**
- **Do not treat codebase context alone as proof of a repository conflict**
